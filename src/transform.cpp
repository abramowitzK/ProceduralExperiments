...
@returns A 4x4 homogenous matrix of floats in column Major order representing
the transformation in world coordinates
*/
...
void Transform::set_scale(Vector3 scale) { mScale = scale; }

void Transform::scale(const float xyz) { mScale += xyz; }

/**
Sets the rotation using Euler angles. This uses a Quaternion internally. User
facing euler angles are expected however.
