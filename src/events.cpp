...
void EventManager::poll_input() {
    ...
    // End of input processing for this frame. Notify all subscribers and run
    // their functions
    notify_subscribers();
}
...
void EventManager::notify_subscribers() {
    // Huge performance savings here. Only notify subscribers if we have to.
    // Just need to remember to always set the dirty bit when we want an intent
    // to be processed right away
    if (mDirty) {
        mDirty = false;
