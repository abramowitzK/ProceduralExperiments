...
void Renderer::render_mesh(MeshRenderer* mesh) {
    ...
    for (const auto& t : mesh->material->tex) {
        glUniform1i(glGetUniformLocation(mesh->material->shader->get_program(),
    ...
    first = GL_TEXTURE0;
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "mvp"), 1, GL_FALSE, glm::value_ptr(vp));
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "m"), 1, GL_FALSE, glm::value_ptr(m));
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "v"), 1, GL_FALSE, glm::value_ptr(v));
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "p"), 1, GL_FALSE, glm::value_ptr(p));
