...
void Aurora::CameraComponent::render(Renderer* renderer) {
    ...
    /// is correctly rotated at the origin. /lookAtVector = XMVector3Normalize(
    /// XMVector3TransformCoord(lookAtVector, rotationMatrix)); /lookAtVector =
    /// glm::normalize(rotationMatrix*lookAtVector); /upVector =
    /// XMVector3Normalize(XMVector3TransformCoord(upVector, rotationMatrix));
