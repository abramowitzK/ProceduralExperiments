...
namespace Aurora {
	...
	void Engine::init(const std::string& initialScene) {
		...
		EventManager::subscribe_to_event(Intents::Escape, std::move((std::function<void()>)[=]() {mPlatform->capture_mouse(!mPlatform->capture); mPlatform->capture = !mPlatform->capture; }));
		EventManager::subscribe_to_event(Intents::Debug, std::move((std::function<void()>)[=]() {mRenderer->debug = !mRenderer->debug; }));
