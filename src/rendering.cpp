...
Renderer::Renderer(int width, int height) {
    ...
    for (GLubyte c = 0; c < 128; c++) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RED,
                     ...
                     mDefaultFont->glyph->bitmap.rows, 0, GL_RED,
                     GL_UNSIGNED_BYTE, mDefaultFont->glyph->bitmap.buffer);
        // Set texture options
        ...
        Character character = {texture,
                               glm::ivec2(mDefaultFont->glyph->bitmap.width,
                                          mDefaultFont->glyph->bitmap.rows),
                               glm::ivec2(mDefaultFont->glyph->bitmap_left,
                                          mDefaultFont->glyph->bitmap_top),
...
void Renderer::create_camera() {
    EventManager::subscribe_to_resize_event(
        [=](int a, int b) { this->handle_resize(a, b); });
...
void Renderer::render_mesh(MeshRenderer* mesh) {
    ...
    vp =
        proj * view *
        mesh->mTransform
            ->get_transform(); // mCamera->projection*mCamera->view*(mesh->mOwner->transform.get_transform());
    ...
    for (const auto& t : mesh->material->tex) {
        glUniform1i(glGetUniformLocation(
                        mesh->material->shader->get_program(),
                        (std::string("tex") + std::to_string(i)).c_str()),
                    i);
    ...
    first = GL_TEXTURE0;
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "mvp"), 1,
        GL_FALSE, glm::value_ptr(vp));
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "m"), 1,
        GL_FALSE, glm::value_ptr(m));
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "v"), 1,
        GL_FALSE, glm::value_ptr(v));
    glUniformMatrix4fv(
        glGetUniformLocation(mesh->material->shader->get_program(), "p"), 1,
...
void Renderer::render_ttf(const std::string& text, float x, float y,
    ...
    apply_render_state(current_render_state, &oldState);
    glm::mat4 projection =
        glm::ortho(0.0f, (float)mWidth, 0.0f, (float)mHeight);
    glUniform3f(
        glGetUniformLocation(mDefaultTtfShader->get_program(), "textColor"),
        color.x, color.y, color.z);
    auto loc =
        glGetUniformLocation(mDefaultTtfShader->get_program(), "projection");
